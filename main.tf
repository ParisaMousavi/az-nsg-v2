...
resource "azurerm_network_security_rule" "this" {
  ...
  source_address_prefixes      = each.value.source_address_prefixes == [] ? null : each.value.source_address_prefixes
  destination_address_prefix   = each.value.destination_address_prefix
  destination_address_prefixes = each.value.destination_address_prefixes == [] ? null : each.value.destination_address_prefixes
}
